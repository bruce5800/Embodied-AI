...
def main():
    """
    使用OpenCV打开摄像头并实时展示画面，同时进行蓝色、黑色和紫色积木块的颜色分拣
    使用HSV颜色空间提高分拣准确率
    BBP = Blue, Black, Purple
    """
    ...
    print("摄像头已打开，蓝色、黑色和紫色积木块分拣程序启动")
    print("按 'q' 键退出")
    ...
    lower_purple = np.array([130, 50, 50])   # 紫色下限
    upper_purple = np.array([160, 255, 255]) # 紫色上限
    ...
    while True:
        ...
        # 在图像上显示检测到的积木块数量统计
        total_objects = blue_objects_count + black_objects_count + purple_objects_count
        ...
        info_text4 = f"Total Blocks: {total_objects}"
        ...
        # 显示原始画面和处理结果
        cv2.imshow('Blue & Black & Purple Block Detection', result_frame)
    ...
    print("摄像头已关闭，蓝色、黑色和紫色积木块分拣程序结束")
