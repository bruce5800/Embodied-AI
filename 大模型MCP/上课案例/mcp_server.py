...
def set_student_servo_angles(
    ...
    def send_packet(ser: serial.Serial, servo_id: int, instruction: int, parameters: List[int]) -> bool:
    ...
    def write_register(ser: serial.Serial, servo_id: int, address: int, value: int, size: int = 2) -> bool:
        params = [address]
        if size == 1:
