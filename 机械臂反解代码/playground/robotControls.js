...
/**
 * 末端笛卡尔空间控制（针对 genkiarm URDF）
 *
 * 末端 link: "zhua"
 * IK 关节:  Rotation, Rotation2, Rotation3, Rotation4, Rotation5（前5个，夹爪不参与）
 *
 * 按键：
 *   ↑ / ↓             — X轴前/后
 *   ← / →             — Y轴左/右
 *   PageUp / PageDown  — Z轴上/下
 */
export function setupCartesianControls(robot) {
  // ── 配置 ─────────────────────────────────────────────────────
  const END_LINK  = 'zhua';
  const IK_JOINTS = ['Rotation', 'Rotation2', 'Rotation3', 'Rotation4', 'Rotation5'];
  const CART_STEP = 0.003;  // 每帧移动步长（米）
  const DELTA     = 1e-5;   // 雅可比数值微分增量
  const MAX_ITER  = 8;      // 单帧最大 IK 迭代次数
  const IK_TOL    = 5e-5;   // 收敛阈值
  const DAMPING   = 0.05;   // 阻尼系数（DLS）

  // ── 按键映射 [dx, dy, dz] ────────────────────────────────────
  const KEY_MAP = {
    'arrowup':    [ CART_STEP, 0, 0],
    'arrowdown':  [-CART_STEP, 0, 0],
    'arrowleft':  [ 0, 0,  CART_STEP],  // 修正方向
    'arrowright': [ 0, 0, -CART_STEP],  // 修正方向
    'pageup':     [ 0,  CART_STEP, 0],  // 修正方向
    'pagedown':   [ 0, -CART_STEP, 0],  // 修正方向
  };

  const keyState = {};
  let activeTimeout = null;
  const cartSection = document.getElementById('cartesianControlSection');

  const setActive = () => {
    cartSection?.classList.add('control-active');
    if (activeTimeout) clearTimeout(activeTimeout);
    activeTimeout = setTimeout(() => cartSection?.classList.remove('control-active'), 2000);
  };
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (KEY_MAP[key]) {
      e.preventDefault();
      keyState[key] = true;
      document.querySelector(`.key[data-key="${key}"]`)?.classList.add('key-pressed');
      setActive();
    }
  });
  window.addEventListener('keyup', (e) => {
    ...
    if (KEY_MAP[key]) {
      keyState[key] = false;
      document.querySelector(`.key[data-key="${key}"]`)?.classList.remove('key-pressed');
    }
  });

  // 鼠标 / 触摸按钮支持：给 HTML 里带 data-cart-key 属性的按钮绑定事件
  function bindCartesianButtons() {
    document.querySelectorAll('[data-cart-key]').forEach(btn => {
      ...
      btn.addEventListener('pointerleave', release);  // 鼠标滑出时也松开
      btn.addEventListener('pointercancel', release);
    });
  }
  // DOM 已就绪时立即绑定；若在 DOMContentLoaded 之前调用则等待
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bindCartesianButtons);
  } else {
    bindCartesianButtons();
  }

  // ── 末端坐标：从 Three.js 世界矩阵读取 ──────────────────────
  function getEndPos() {
    if (!robot) return [0, 0, 0];
    const link = robot.links?.[END_LINK];
    if (link?.matrixWorld) {
      const e = link.matrixWorld.elements;
      return [e[12], e[13], e[14]];
    }
    let found = null;
    robot.traverse?.((obj) => { if (obj.name === END_LINK) found = obj; });
    if (found?.matrixWorld) {
  ...
  // ── 读取 / 写入 IK 关节角度 ──────────────────────────────────
  function getAngles() {
    return IK_JOINTS.map(n => robot?.joints?.[n]?.angle ?? 0);
  }

  // applyAngles 同时把 clamp 后的值写回传入数组，保证一致性
  function applyAngles(angles) {
    IK_JOINTS.forEach((name, i) => {
      const joint = robot?.joints?.[name];
      if (!joint) return;
      let val = angles[i];
      if (joint.limit && joint.jointType !== 'continuous') {
        val = Math.max(joint.limit.lower, Math.min(joint.limit.upper, val));
        angles[i] = val; // 写回，避免 clamp 后数组与关节值不一致
      }
  ...
  // ── 数值雅可比（3×n，按列存储）──────────────────────────────
  // 重要：每列扰动计算完后立即还原到 baseAngles，避免列间相互污染
  function computeJacobian(baseAngles) {
    applyAngles([...baseAngles]);            // 确保从基准状态出发
    const basePos = getEndPos();
    return IK_JOINTS.map((_, i) => {
      const p = [...baseAngles];
      p[i] += DELTA;
      applyAngles(p);
      const np = getEndPos();
      applyAngles([...baseAngles]);          // 每列后都还原
      return [
        (np[0] - basePos[0]) / DELTA,
        (np[1] - basePos[1]) / DELTA,
        (np[2] - basePos[2]) / DELTA,
      ];
    });
  }
  ...
  function dls(J, err) {
    ...
    if (Math.abs(det) < 1e-14) return new Array(n).fill(0);
    ...
    return J.map(col => col[0]*y[0] + col[1]*y[1] + col[2]*y[2]);
  }

  // ── UI 刷新 ───────────────────────────────────────────────────
  function updateUI(pos, status, color) {
    const posEl = document.getElementById('endEffectorPos');
    const stEl  = document.getElementById('ikStatus');
    if (posEl) posEl.textContent = `${pos[0].toFixed(3)} ${pos[1].toFixed(3)} ${pos[2].toFixed(3)}`;
    if (stEl)  { stEl.textContent = status; stEl.style.color = color; }
  }

  // ── 主更新函数（在渲染循环中调用）──────────────────────────
  let _diagDone = false; // 只在第一次按键时打印一次诊断信息

  function updateCartesian() {
    if (!robot?.joints) return;

    let dx = 0, dy = 0, dz = 0, anyKey = false;
    for (const [key, d] of Object.entries(KEY_MAP)) {
      if (keyState[key]) { dx += d[0]; dy += d[1]; dz += d[2]; anyKey = true; }
    }

    const curPos = getEndPos();

    if (!anyKey) {
      updateUI(curPos, '就绪', 'rgba(255,255,255,0.5)');
      return;
    }

    // 第一次按键时打印诊断，帮助排查关节/末端读取是否正常
    if (!_diagDone) {
      _diagDone = true;
      console.log('[CartesianIK] 诊断信息:');
      console.log('  末端link (zhua) 位置:', curPos);
      IK_JOINTS.forEach(name => {
        const j = robot.joints[name];
        console.log(`  关节 ${name}: angle=${j?.angle?.toFixed(4)}, type=${j?.jointType}, exists=${!!j}`);
      });
      console.log('  robot.links keys:', Object.keys(robot.links ?? {}));
    }

    const targetPos = [curPos[0]+dx, curPos[1]+dy, curPos[2]+dz];
    let angles = getAngles();
    let solved = false, lastErr = 0;

    for (let iter = 0; iter < MAX_ITER; iter++) {
      applyAngles(angles);
      const pos = getEndPos();
      const err = [targetPos[0]-pos[0], targetPos[1]-pos[1], targetPos[2]-pos[2]];
      lastErr = Math.hypot(err[0], err[1], err[2]);
      if (lastErr < IK_TOL) { solved = true; break; }
      // 计算雅可比（内部自带还原，返回基于当前 angles 的微分）
      const J = computeJacobian([...angles]);
      const dTheta = dls(J, err);
      angles = angles.map((a, i) => a + dTheta[i]);
    }
    ...
    if (isConnectedToRealRobot) {
      IK_JOINTS.forEach((name, i) => {
        const servoId = i + 1;
        if (servoCommStatus[servoId]?.status === 'error') return;
        const joint = robot.joints[name];
        if (!joint) return;
        const stepChange = Math.round((angles[i] - (joint.angle ?? 0)) * (4096 / (2 * Math.PI)));
        if (stepChange === 0) return;
        const prev = servoCurrentPositions[servoId];
        const next = Math.max(0, Math.min(4095, prev + stepChange));
        servoCurrentPositions[servoId] = next;
        writeServoPosition(servoId, next)
          .then(ok => {
            if (ok) { servoLastSafePositions[servoId] = next; servoCommStatus[servoId].status = 'success'; }
            else    { servoCurrentPositions[servoId] = prev; }
            updateServoStatusUI();
          })
          .catch(() => { servoCurrentPositions[servoId] = prev; });
      });
    }

    updateUI(getEndPos(),
      solved ? '✓ 已收敛' : `误差 ${lastErr.toFixed(4)}`,
      solved ? '#4CAF50' : '#FF9800'
    );
  }
