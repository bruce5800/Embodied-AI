...
// Import forward kinematics module
...
/**
 * 更新末端坐标显示
 * @param {Object} robot - 机器人对象，包含关节信息
 */
export function updateEndEffectorPosition(robot) {
  try {
    // 获取当前6个关节的角度（弧度）
    const jointAngles = [];
    
    // 从机器人对象中提取关节角度
    // 根据robotConfig.js中的映射，关节顺序为：
    // 0: 腰部旋转 (Rotation)
    // 1: 大臂 (Rotation2) 
    // 2: 小臂 (Rotation3)
    // 3: 腕部 (Rotation4)
    ...
    if (robot && robot.joints) {
      // 按照URDF中的关节顺序获取角度
      const jointNames = ['Rotation', 'Rotation2', 'Rotation3', 'Rotation4', 'Rotation5', 'Rotation6'];
      
      for (let i = 0; i < jointNames.length; i++) {
        const joint = robot.joints[jointNames[i]];
        if (joint) {
          jointAngles.push(joint.angle || 0);
        } else {
          jointAngles.push(0);
        }
      }
      ...
      // 如果没有机器人对象，使用默认值
      jointAngles.push(0, 0, 0, 0, 0, 0);
    }
    
    // 使用正运动学计算末端位置
    const endEffectorPosition = forwardKinematics.forwardKinematics(jointAngles);
    ...
    if (xElement) {
      xElement.textContent = (endEffectorPosition[0] * 1000).toFixed(1); // 转换为毫米
    }
    if (yElement) {
      yElement.textContent = (endEffectorPosition[1] * 1000).toFixed(1); // 转换为毫米
    }
    if (zElement) {
      zElement.textContent = (endEffectorPosition[2] * 1000).toFixed(1); // 转换为毫米
    }
    
  } catch (error) {
    console.error('更新末端坐标时出错:', error);
    ...
    if (xElement) xElement.textContent = 'Error';
    if (yElement) yElement.textContent = 'Error';
    if (zElement) zElement.textContent = 'Error';
  }
}