...
// 导入控制工具函数
import { setupKeyboardControls, setupControlPanel, updateEndEffectorPosition } from './robotControls.js';
import { inverseKinematics } from './inverseKinematics.js';
import { PositionController } from './positionControl.js';
...
function init() {
  ...
  function loadModelFromHash() {
    ...
    manager.onLoad = () => {
      ...
      positionController = new PositionController(window.robot, inverseKinematics);
...
function render() {
  ...
  if (window.robot) {
    updateEndEffectorPosition(window.robot);
  }
