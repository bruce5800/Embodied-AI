...
class GenkiArmInverseKinematics:
    def __init__(self):
        ...
        self.position_tolerance = 1e-4  # 位置误差容忍度 (米)
        self.step_size = 0.1           # 迭代步长
        self.damping_factor = 0.01     # 阻尼因子，防止奇异点
    ...
    def compute_jacobian(self, joint_angles: List[float]) -> np.ndarray:
        ...
        # 计算当前末端位置
        current_pos = self.fk.forward_kinematics(joint_angles)
        ...
        for i in range(6):
            ...
            # 计算扰动后的末端位置
            perturbed_pos = self.fk.forward_kinematics(perturbed_angles)
    ...
    def inverse_kinematics(self, target_position: List[float], 
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                damped_inverse = np.linalg.inv(JJT + self.damping_factor * np.eye(3))
                ...
                # 增加阻尼因子重试
                JJT = np.dot(jacobian, jacobian.T)
                damped_inverse = np.linalg.inv(JJT + (self.damping_factor * 10) * np.eye(3))
                delta_angles = self.step_size * np.dot(jacobian.T, np.dot(damped_inverse, position_error))
