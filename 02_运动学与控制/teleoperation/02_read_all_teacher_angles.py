...
def main():
    """主函数 - 读取4个舵机的角度"""
    # 配置参数
    port = 'COM8'  # 根据实际情况修改串口
    motor_ids = [1, 2, 3, 4,5,6]  # 要读取的舵机ID列表
    ...
    try:
        ...
        print(f"开始读取舵机ID {motor_ids} 的角度，按 Ctrl+C 停止")
        print("---")
        ...
        while True:
            ...
            for motor_id, angle in angles.items():
                if angle is not None:
                    print(f"舵机ID {motor_id} 当前角度: {angle:.2f}°", end=" | ")
                else:
                    print(f"舵机ID {motor_id} 读取失败", end=" | ")
            print()  # 换行
